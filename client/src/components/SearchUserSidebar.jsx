...
const SearchedUsersResultCard = ({ user, opponentOnline }) => {
  ...
  const handleCreateChat = async () => {
    try {
      // Attempt to create (or get) a one-to-one chat with the user
      const chat = await createOneToOneChat(user._id);
      
      // Fix: If the returned object has a 'data' property, use it
      const chatData = chat && chat.data ? chat.data : chat;
      
      console.log("Chat object returned:", chatData);
      
      // If the chat already exists, notify the user
      if (chatData && chatData.existing) {
