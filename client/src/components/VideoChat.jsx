...
export default function VideoChat({ show }) {
  ...
  return (
    <div
      className={`absolute ${show ? "" : "hidden"} z-50 bg-slate-800 w-full h-full bg-opacity-80 p-20 md:p-0`}
    >
      <div className="relative bg-slate-900 w-full h-full rounded-md">
        ...
        <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 flex space-x-4 p-2">
          ...
          <div className="relative flex items-center bg-gray-700 rounded-full">
            {showAudioOptionTray && !isMobileDevice && (
              <div className="videoDevices absolute bottom-10 bg-white px-2 py-1">
                {inputAudioDevices?.map((device) => (
                  <li
                    ...
                    className={`text-xs w-[250px] text-left cursor-pointer my-2 ${
                      device.deviceId === selectedInputAudioDevice ? "text-blue-700" : ""
                    }`}
                  >
                    {device.label}
                  </li>
            ...
            {!isMobileDevice && (
              <div
                ...
                className="px-1 text-2xl text-white cursor-pointer"
              >
                {showAudioOptionTray ? <RiArrowDropUpLine /> : <RiArrowDropDownLine />}
              </div>
            )}
            <button
          ...
          {/* Camera Controls */}
          <div className="relative flex items-center bg-gray-700 rounded-full">
            {showVideoOptionTray && !isMobileDevice && (
              <div className="videoDevices absolute bottom-10 bg-white px-2 py-1">
                {inputVideoDevices?.map((device) => (
                  <li
                    ...
                    className={`text-xs w-[250px] text-left cursor-pointer my-2 ${
                      device.deviceId === selectedInputVideoDevice ? "text-blue-700" : ""
                    }`}
                  >
                    {device.label}
                  </li>
            ...
            {!isMobileDevice && (
              <div
                ...
                className="px-1 text-2xl text-white cursor-pointer"
              >
                {showVideoOptionTray ? <RiArrowDropUpLine /> : <RiArrowDropDownLine />}
              </div>
            )}
            <button
          ...
          {/* Flip Camera and Hangup Buttons */}
          <button
            onClick={flipCamera}
            className="p-2 bg-white bg-opacity-40 rounded-full text-white hover:bg-opacity-50"
          >
